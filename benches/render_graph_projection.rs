...
use git2::Oid;
use guitar::{
    core::{chunk::NONE, graph_service::GraphRow, renderers::render_graph_projection as render_graph_projection_lines},
...
fn render_case(
    theme: &Theme, symbols: &SymbolTheme, rows: &[guitar::core::graph_service::GraphRow], history: &guitar::core::graph_service::GraphHistory, head_alias: u32, start: usize, end: usize,
...
fn render_graph_projection_uncommitted_row(bencher: Bencher) {
    ...
    let rendered = render_case(&fixture.theme, &fixture.symbols, &rows, &fixture.history, fixture.head_alias, 0, rows.len(), true);

    bencher
        .counter(ItemsCount::new(rows.len()))
        .counter(BytesCount::new(rendered.1))
        .bench(|| black_box(render_case(&fixture.theme, &fixture.symbols, &rows, &fixture.history, fixture.head_alias, 0, rows.len(), true)));
}
...
fn render_graph_projection_ascii_theme(bencher: Bencher) {
    ...
    let rendered = render_case(&theme, &symbols, &fixture.rows, &fixture.history, fixture.head_alias, 4, fixture.rows.len(), false);

    bencher
        .counter(ItemsCount::new(fixture.rows.len().saturating_sub(4)))
        .counter(BytesCount::new(rendered.1))
        .bench(|| black_box(render_case(&theme, &symbols, &fixture.rows, &fixture.history, fixture.head_alias, 4, fixture.rows.len(), false)));
}
