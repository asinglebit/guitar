...
fn pane_window_rows(pane: GraphPane, walk_ctx: &Walker, start: usize, end: usize) -> (usize, Vec<GraphPaneRow>) {
    match pane {
        ...
        GraphPane::Stashes => stash_pane_rows(walk_ctx, start, end),
        GraphPane::Reflogs => reflog_pane_rows(walk_ctx, start, end),
    }
}

fn stash_pane_rows(walk_ctx: &Walker, start: usize, end: usize) -> (usize, Vec<GraphPaneRow>) {
...
fn visible_reflogs(walk_ctx: &Walker) -> impl Iterator<Item = (u32, &HeadReflogEntry)> {
    walk_ctx.head_reflog_entries.iter().filter_map(|entry| walk_ctx.oids.get_existing_alias(entry.new_oid).map(|alias| (alias, entry)))
}

fn reflog_pane_rows(walk_ctx: &Walker, start: usize, end: usize) -> (usize, Vec<GraphPaneRow>) {
    let total = visible_reflogs(walk_ctx).count();
    let window = pane_window(start, end, total);
    let selected_reflogs = || visible_reflogs(walk_ctx).skip(window.start).take(window.len());
