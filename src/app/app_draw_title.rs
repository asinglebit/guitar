...
impl App {
    pub fn draw_title(&mut self, frame: &mut Frame) {
        ...
        let hint =
            if self.mode == InputMode::Action { Span::styled("action ", Style::default().fg(self.theme.COLOR_GRASS)) } else { Span::styled("normal ", Style::default().fg(self.theme.COLOR_GREY_700)) };
        let paragraph = ratatui::widgets::Paragraph::new(Line::from(hint)).right_aligned().block(Block::default());
