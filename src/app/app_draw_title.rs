...
impl App {
    pub fn draw_title(&mut self, frame: &mut Frame) {
        ...
        let folder = Span::styled(
            format!(" {SYM_FOLDER} {}", truncate_start_with_ellipsis(path.as_str(), available_width)),
        ...
        let hint_line = Line::from(Span::styled(
            format!("{} ", focus_name),
            Style::default().fg(self.theme.COLOR_GREY_700),
        ));

        let paragraph = ratatui::widgets::Paragraph::new(hint_line)
            .right_aligned()
            .block(Block::default());
