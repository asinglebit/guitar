...
impl App {
    pub fn draw_tags(&mut self, frame: &mut Frame) {
        ...
        self.request_pane_window(GraphPane::Tags, preload_start, preload_end);
        ...
        if let Some(rows) = self.graph.tags_window.as_ref().and_then(|window| aligned_pane_rows(window, start, end)) {
            ...
            for row in rows {
                if let Some(GraphPaneRow::Tag { name, lane, .. }) = row {
                    ...
                    lines.push(Line::from(Span::styled(format!("{} {truncated}", self.symbols.entity.tag), Style::default().fg(color))));
                } else {
        ...
        if lines.is_empty() && (!lines_are_windowed || known_empty) {
            ...
            let empty_text = format!("{} {}", self.symbols.empty_state.mark, empty::NO_TAGS());
        ...
        let list_items =
            zebra_list_items(&lines[display_start..display_end], visible_height, start, self.tags_selected, self.focus == Focus::Tags, !tags_empty, self.cursor_line_background(), &self.theme);
        ...
        if self.layout_config.is_branches || self.layout_config.is_tags {
            let top_border = Paragraph::new(self.symbols.border.horizontal.repeat(self.layout.tags.width.saturating_sub(1) as usize)).style(Style::default().fg(self.theme.COLOR_BORDER));
        ...
        let mut scrollbar_state = ScrollbarState::new(scroll_range).position(self.tags_scroll.get());
        let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
            .begin_symbol(Some(if self.layout_config.is_branches { self.symbols.border.vertical.as_str() } else { self.symbols.border.horizontal.as_str() }))
