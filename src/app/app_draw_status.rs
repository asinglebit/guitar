    ...
    pub fn draw_status(&mut self, frame: &mut Frame) {
        ...
        if is_showing_uncommitted {
            ...
            if lines_status_top.is_empty() {
                ...
                lines_status_top
                    .push(Line::from(Span::styled(center_line(&truncate_with_ellipsis("⊘ no staged changes", max_text_width), max_text_width + 3), Style::default().fg(self.theme.COLOR_GREY_800))));
            } else {
            ...
            if lines_status_bottom.is_empty() {
                ...
                lines_status_bottom
                    .push(Line::from(Span::styled(center_line(&truncate_with_ellipsis("⊘ no unstaged changes", max_text_width), max_text_width + 3), Style::default().fg(self.theme.COLOR_GREY_800))));
            } else {
